...
def get_ticker_from_google(company_name: str, ticker_hint: str = "") -> str | None:
    ...
    3. Return ticker dalam format Google Finance, e.g. "SUPA.JK"
    """
    # ── Coba verifikasi ticker_hint dulu (lebih cepat) ──
    if ticker_hint and re.match(r"^[A-Z]{2,6}$", ticker_hint.strip().upper()):
        candidate = ticker_hint.strip().upper() + ".JK"
        if _verify_ticker_on_google_finance(candidate):
            logger.info(f"Ticker hint terverifikasi: {candidate}")
            return candidate

    # ── Fallback: Google Search ──
    query = f"{company_name} kode saham IDX BEI site:finance.yahoo.com OR site:google.com/finance"
    ...
    try:
        ...
        # Pola pencarian ticker di teks hasil Google
        patterns = [
            r"IDX\s*[:\-]\s*([A-Z]{2,6})",
            r"BEI\s*[:\-]\s*([A-Z]{2,6})",
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                ticker = found + ".JK" if not found.endswith(".JK") else found
...
def _verify_ticker_on_google_finance(ticker: str) -> bool:
    """Cek apakah ticker valid dengan hit Google Finance dan pastikan ada data harga."""
    exchange, symbol = _parse_ticker(ticker)
    url = f"https://www.google.com/finance/quote/{symbol}:{exchange}"
    try:
        resp = requests.get(url, headers=HEADERS, timeout=TIMEOUT)
        # Jika halaman ada harga, anggap valid
...
def _parse_page(html: str, exchange: str) -> dict:
    ...
    # ── Key stats (Market Cap, Shares Outstanding, dll) ──
    stats = {}
    # Google Finance key stats ada di div dengan class tertentu
    # Scrape semua pasangan label-value yang ada di halaman
    ...
    # Fallback jika class berubah: scrape semua <tr> atau list item
    if not stats:
