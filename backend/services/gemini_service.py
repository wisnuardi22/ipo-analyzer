...
def _build_mega_prompt(text: str, is_pro: bool, is_banking: bool,
    ...
    risk_count    = "6-8" if is_pro else "4-5"
    benefit_count = "5-7" if is_pro else "3-4"
    summary_len   = "3-4 sentences covering company profile, deep business model, and IPO basics" if is_pro else "2 sentences covering company profile and IPO basics"
    uof_desc      = "DEEP DIVE. Include exact nominal amounts, target subsidiaries, and specific project names (e.g. 'Rp 215B for 99.9% acquisition of PT XYZ'). Do NOT be vague." if is_pro else "Brief but accurate — include nominal amounts where stated."
    risk_desc     = "Provide DEEP analytical reasoning. Evaluate strictly — if intense competition, high debt, or severe regulatory threats exist, classify 1-2 as 'High'." if is_pro else "Brief but specific descriptions."
...
1. Find table: "IKHTISAR DATA KEUANGAN PENTING" / "LAPORAN LABA RUGI" / "SELECTED FINANCIAL DATA"
   Read column headers → those are the actual years in this document (use EXACT years).
   For EACH year extract:
   - pendapatan  : Total Revenue / Net Revenue / Penjualan Bersih / Net Interest Income (bank)
   - laba_kotor  : Gross Profit (NULL for banks)
...
4. IPO data: total_saham_beredar (post-IPO shares, number only), harga_penawaran (number only, null if DRHP)
...
5. IDENTITY:
   ...
   - ipo_date: listing date as written in document
   - share_price: EXACT string e.g. "Rp 500" — NO spelled-out words in parentheses
   - total_shares: EXACT numeric string e.g. "5,000,000,000" — NO spelled-out words
   - market_cap: calculate shares × price, formatted (e.g. "Rp 2.80 Triliun") or "N/A"

6. SUMMARY: {summary_len}. Write in English with specific numbers from document.
...
7. USE OF PROCEEDS — MUST BE ACCURATE:
   ...
   - allocation: EXACT percentage from document (total MUST equal 100)
   Min 2, max 6 items.

8. UNDERWRITER:
   - lead: full name of lead underwriter
   - others: array of co-underwriter names ([] if none)
   - type: "Full Commitment" or "Best Efforts"
   - reputation: 1-2 sentences in English about their track record

9. RISK FACTORS — MUST HAVE VARIED LEVELS:
   Find "Faktor Risiko" / "Risk Factors". Extract {risk_count} most important risks.
   {risk_desc}

   LEVEL RULES (STRICT):
   - "High": Directly threatens business survival — license revocation, going concern doubt,
     single customer >50% revenue, material debt default, critical failed acquisition
   - "Medium": Significant but manageable — technology risk, competition, regulatory changes,
     credit/NPL risk, key person dependency, cybersecurity, operational risk
   - "Low": General market risks — share price volatility, minor FX, force majeure, macro conditions

   DISTRIBUTION: 0-2 High, 2-3 Medium, 1-2 Low. Assess THIS company independently.
   overall_risk_level: ONE WORD ONLY — "High", "Medium", or "Low"
...
def _compute_kpi(fin_section: Dict, fx_rate, is_banking: bool, currency: str, unit: str):
    ...
    # Fallback if no ratio table found
    try:
...
def analyze_prospectus(text: str, lang: str="ID", model: str=None) -> dict:
    ...
    Output always in English. lang param kept for API compatibility.
    """
    ...
    for r in parsed.get("risks") or []:
        ...
        if is_pro:
            # Pro: trust LLM fully — it has better context
            final_lvl = llm_lvl
        else:
            # Basic: use keyword scoring as guardrail
