...
# ══════════════════════════════════════════════════════
# STEP 1: EKSTRAK ANGKA MENTAH VIA LLM + REGEX FALLBACK
# ══════════════════════════════════════════════════════
...
- Jika tidak ada di potongan ini → isi null/[]"""
    ...
    # Kirim 3 chunk paling relevan (awal, tengah, akhir)
    chunks = chunk_text(text, max_len=15000, overlap=500)
    # Prioritas: cari chunk yang mengandung kata kunci keuangan
    priority_chunks = []
    ...
    # Tambahkan chunk pertama dan terakhir
...
# ══════════════════════════════════════════════════════
# STEP 2: NORMALISASI & HITUNG METRIK
# ══════════════════════════════════════════════════════
    ...
    try:
        if saham and laba_bersih and float(saham) > 0:
            ...
            if harga and eps_val != 0:
                # Konversi ke satuan yang sama
                if currency == "USD" and fx_rate and fx_rate > 0:
                    price_ccy = float(harga) / fx_rate  # Rp → USD
                else:
...
def analyze_prospectus(text: str) -> dict:
    ...
    # 2. Ekstrak angka mentah via LLM (multi-chunk)
